...
c---------------------------------------------------------------------------c
      ...
      external timer_read
      double precision t, t0, tinit, mflops, timer_read, tt(1)
...
c---------------------------------------------------------------------
 ...
 51         format(' No input file. Using compiled defaults ')
      ...
      if (me .eq. root .and. nprocs .gt. 1) then
          sh_ibuf(1) = lt
          sh_ibuf(2) = nit
          sh_ibuf(3) = nx(lt)
          sh_ibuf(4) = ny(lt)
          sh_ibuf(5) = nz(lt)
          sh_ibuf(6:13) = debug_vec(0:7)
      end if

      sync all

      if (me .ne. root) then
          sh_ibuf(1:13) = sh_ibuf(1:13)[root]
          lt      = sh_ibuf(1)
          nit     = sh_ibuf(2)
          nx(lt)  = sh_ibuf(3)
          ny(lt)  = sh_ibuf(4)
          nz(lt)  = sh_ibuf(5)
          debug_vec(0:7)  = sh_ibuf(6:13)
      end if
      ...
      if( .not. dead(kk) )then
         do  axis = 1, 3
            if( nprocs .ne. 1) then
               ...
               if (nbr(axis,-1,kk) == nbr(axis,+1,kk)) then
                   sync images( nbr(axis,-1,kk)+1 )
               else
                   sync images( [nbr(axis,-1,kk)+1, nbr(axis,+1,kk)+1] )
               end if
...
c---------------------------------------------------------------------
      ...
      double precision ten( mm, 0:1 ), temp(1), best
