...
c---------------------------------------------------------------------
c  Get receive starts from reduce exchange partners
c---------------------------------------------------------------------
      sync all
      do i = 1, l2npcols
        reduce_rrecv_starts(i) =
     >          reduce_recv_starts(i)[reduce_exch_proc(i)]
      end do
...
c---------------------------------------------------------------------
         ...
         do i = 1, l2npcols
            ...
            sync images( reduce_exch_proc(i)+1 )
            norm_temp1(1:2)[reduce_exch_proc(i)] = norm_temp1(1:2)
            sync images( reduce_exch_proc(i)+1 )
...
c---------------------------------------------------------------------
      ...
      u(1) = t
      call coreduce_r8(CAF_MAX, u, result_image=root+1)
      tmax = u(1)
...
c---------------------------------------------------------------------
         do i = l2npcols, 1, -1
            ...
            recv_start_idx = reduce_rrecv_starts(i)
            recv_length = reduce_send_lengths(i)
