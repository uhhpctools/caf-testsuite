...
c  ... broadcast parameters
       if (node .eq. root .and. no_nodes .gt. 1) then
          ...
          sh_ibuf(5) = rd_interval
          sh_ibuf(6) = wr_interval
          sh_ibuf(7) = collbuf_nodes
          sh_ibuf(8) = collbuf_size
          sh_ibuf(9) = itimer
          sh_rbuf(1) = dt
       endif
       sync all
       if (node .ne. root) then
          ...
          rd_interval    = sh_ibuf(5)[root]
          wr_interval    = sh_ibuf(6)[root]
          collbuf_nodes  = sh_ibuf(7)[root]
          collbuf_size   = sh_ibuf(8)[root]
          itimer         = sh_ibuf(9)[root]
          dt             = sh_rbuf(1)[root]
          timeron = (itimer.ne.0)
       endif
...
c---------------------------------------------------------------------
 ...
 200            format(' Time step ', i4)
       ...
       if (iotype .ne. 0) then
          ...
          t1(1) = t
          call coreduce_r8(caf_sum, t1(1:1), tming(1:1))
          tiominv = tming(1)
       endif
