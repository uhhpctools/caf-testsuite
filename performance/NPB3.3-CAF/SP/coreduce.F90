      module custom_coreduce
        ...
        public :: coreduce_i4, coreduce_i8
        public :: coreduce_r4, coreduce_r8
        ...
        contains
           ...
           !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
           ! Reduction for 4-byte Integer Arrays
           !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

           subroutine coreduce_i4_to_image( op, source, result_image,  &
                                           work_buf, sz)
               implicit none
               integer :: op
               integer :: sz
           ...
           end subroutine coreduce_i4_to_image

            subroutine coreduce_i4_to_all(op, B, work, N)
                implicit none
                integer                 :: op
                integer                 :: N
            ...
            end subroutine coreduce_i4_to_all

            subroutine coreduce_i4( op, source, result, result_image )
                implicit none
                integer :: op
                integer :: source(:)
                ...
                integer, save :: work_buf(WORK_BUF_SIZE)[*]
                integer, allocatable :: alt_work_buf(:)[:]
                integer, allocatable :: contig_src(:)
                ...
                if ( present(result) .and. present(result_image) ) then
                    ...
                    if (.false.) then
                        ...
                        contig_src(:) = source(:)
                        if (size(source) > WORK_BUF_SIZE) then
                            call coreduce_i4_to_image(op, contig_src, &
                                                     result_image,   &
                                                     alt_work_buf,   &
                                                     size(contig_src))
                        else
                    ...
                    if (.false.) then
                        ...
                        contig_src(:) = source(:)
                        if (size(source) > WORK_BUF_SIZE) then
                            call coreduce_i4_to_all(op, contig_src, &
                                                   alt_work_buf,   &
                                                   size(contig_src))
                        else
                            call coreduce_i4_to_all(op, contig_src, &
            ...
            end subroutine coreduce_i4

           !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
           ! Reduction for 8-byte Integer Arrays
           !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

           subroutine coreduce_i8_to_image( op, source, result_image,  &
                                           work_buf, sz)
               implicit none
               integer :: op
               integer :: sz
               integer (kind=8) :: source(sz)
               integer (kind=8) :: work_buf(sz)[*]
               integer :: result_image
           ...
           end subroutine coreduce_i8_to_image

            subroutine coreduce_i8_to_all(op, B, work, N)
                implicit none
                integer                 :: op
                integer                 :: N
                integer (kind=8), intent(inout)  :: B(N)
                integer (kind=8)        :: work(N)[*]
            ...
            end subroutine coreduce_i8_to_all

            subroutine coreduce_i8( op, source, result, result_image )
                implicit none
                integer :: op
                ...
                integer (kind=8), save :: work_buf(WORK_BUF_SIZE)[*]
                integer (kind=8), allocatable :: alt_work_buf(:)[:]
                ...
                if ( present(result) .and. present(result_image) ) then
                    ...
                    if (.false.) then
                        ...
                        contig_src(:) = source(:)
                        if (size(source) > WORK_BUF_SIZE) then
                            call coreduce_i8_to_image(op, contig_src, &
                                                     result_image,   &
                                                     alt_work_buf,   &
                                                     size(contig_src))
                        else
                    ...
                    if (.false.) then
                        ...
                        contig_src(:) = source(:)
                        if (size(source) > WORK_BUF_SIZE) then
                            call coreduce_i8_to_all(op, contig_src, &
                                                   alt_work_buf,   &
                                                   size(contig_src))
                        else
                            call coreduce_i8_to_all(op, contig_src, &
            ...
            end subroutine coreduce_i8

           !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
           ! Reduction for 4-byte Real Arrays
           !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

           subroutine coreduce_r4_to_image( op, source, result_image,  &
                                           work_buf, sz)
               implicit none
               integer :: op
               integer :: sz
               real (kind=4) :: source(sz)
               real (kind=4) :: work_buf(sz)[*]
               integer :: result_image
           ...
           end subroutine coreduce_r4_to_image

            subroutine coreduce_r4_to_all(op, B, work, N)
                implicit none
                integer                 :: op
                integer                 :: N
            ...
            end subroutine coreduce_r4_to_all

            subroutine coreduce_r4( op, source, result, result_image )
                implicit none
                integer :: op
                real (kind=4) :: source(:)
                ...
                real (kind=4), save :: work_buf(WORK_BUF_SIZE)[*]
                real (kind=4), allocatable :: alt_work_buf(:)[:]
                ...
                if ( present(result) .and. present(result_image) ) then
                    ...
                    if (.false.) then
                        ...
                        contig_src(:) = source(:)
                        if (size(source) > WORK_BUF_SIZE) then
                            call coreduce_r4_to_image(op, contig_src, &
                                                     result_image,   &
                                                     alt_work_buf,   &
                                                     size(contig_src))
                        else
                    ...
                    if (.false.) then
                        ...
                        contig_src(:) = source(:)
                        if (size(source) > WORK_BUF_SIZE) then
                            call coreduce_r4_to_all(op, contig_src, &
                                                   alt_work_buf,   &
                                                   size(contig_src))
                        else
                            call coreduce_r4_to_all(op, contig_src, &
