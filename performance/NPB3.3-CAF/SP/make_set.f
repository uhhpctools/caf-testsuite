...
c      array of cells
c---------------------------------------------------------------------
       cell_coord(1,1) = mod(node-1,p) 
       cell_coord(2,1) = (node-1)/p 
       cell_coord(3,1) = 0
...
c---------------------------------------------------------------------
       ...
       predecessor(1) = mod(i-1+p,p) + p*j + 1
       predecessor(2) = i + p*mod(j-1+p,p) + 1
       predecessor(3) = mod(i+1,p) + p*mod(j-1+p,p) + 1
       successor(1)   = mod(i+1,p) + p*j + 1
       successor(2)   = i + p*mod(j+1,p) + 1
       successor(3)   = mod(i-1+p,p) + p*mod(j+1,p) + 1

       sync_pred(1,:) = predecessor(:)
       sync_succ(1,:) = successor(:)
       if (ncells .gt. 2) then
          sync_pred(2,:) = successor(:)
          sync_succ(2,:) = predecessor(:)
       endif
