...
c---------------------------------------------------------------------
          ...
          double complex       :: pad1(3)[*]
          double complex       :: u1(ntdivnp)[*]
          double complex       :: pad2(3)[*]
          double complex       :: u2(ntdivnp)[*]
          double complex       :: pad3(3)[*]
          double precision     :: twiddle(ntdivnp)[*]

          integer, allocatable :: row_images(:)
          integer, allocatable :: col_images(:)
          integer              :: sh_ibuf(4)[0:*]
          logical              :: sh_lbuf(1)[0:*]
          contains
...
c---------------------------------------------------------------------
      ...
      return
      end subroutine transpose2_local
...
c---------------------------------------------------------------------
      ...
      double complex xin(ntdivnp)[0:np1-1,0:*]
      double complex xout(ntdivnp)[0:np1-1,0:*]
      integer ierr
      integer :: i, q
...
!      if (timers_enabled) call synchup()
      ...
      do i = 1, np2
        ...
        end_send    = (ntdivnp/np)*(q+1)
        start_recv  = (ntdivnp/np)*(me2)+1
        end_recv    = (ntdivnp/np)*(me2+1)
      ...
      return
      end subroutine transpose2_global
...
c---------------------------------------------------------------------
      ...
      return
      end subroutine transpose2_finish
...
c---------------------------------------------------------------------
 200  continue
      ...
      if (timers_enabled) call timer_stop(T_transxzloc)
      return 
      end subroutine transpose_x_z_local
...
c---------------------------------------------------------------------
      ...
      double complex xin(d3*d2*d1)[0:np1-1,0:*]
      double complex xout(d3*d2*d1)[0:np1-1,0:*] ! not real layout, but right size
...
c---------------------------------------------------------------------
      ...
      do i = 1, np2
        q           = mod(me2+i,np2)
        start_send  = (d1*d2*d3/np)*(q)+1
        end_send    = (d1*d2*d3/np)*(q+1)
        start_recv  = (d1*d2*d3/np)*(me2)+1
        end_recv    = (d1*d2*d3/np)*(me2+1)
        xout(start_recv:end_recv)[me1,q] = xin(start_send:end_send)
      end do
      ...
      if (timers_enabled) call timer_stop(T_transxzglo)
      return
      end subroutine transpose_x_z_global
...
c---------------------------------------------------------------------
      ...
      if (timers_enabled) call timer_stop(T_transxzfin)
      return
      end subroutine transpose_x_z_finish
...
c---------------------------------------------------------------------
      ...
      return 
      end subroutine transpose_x_y_local
...
c---------------------------------------------------------------------
      double complex xin(d2*d3*d1)[0:np1-1,0:*]
      double complex xout(d2*d3*d1)[0:np1-1,0:*] ! not real layout but right size
      integer ierr
      integer :: i, q
      integer :: start_send, end_send, start_recv, end_recv
...
c---------------------------------------------------------------------
c do transpose among all processes with same 2-coord (me2)
c---------------------------------------------------------------------
      ...
      do i = 1, np1
        ...
        end_send    = (d1*d2*d3/np1)*(q+1)
        start_recv  = (d1*d2*d3/np1)*(me1)+1
      ...
      return
      end subroutine transpose_x_y_global
...
c---------------------------------------------------------------------
      ...
      return
      end subroutine transpose_x_y_finish
...
!      double complex   u0(ntdivnp)[*], u1(ntdivnp)[*], u2(ntdivnp)[*]
!      double precision twiddle(ntdivnp)
!
...
c---------------------------------------------------------------------
      ...
      subroutine finish_up
          ...
          deallocate( row_images, col_images)

      end subroutine finish_up
...
c---------------------------------------------------------------------
      if (me .eq. 0 .and. np .gt. 1) then
          sh_ibuf(1) = np1
          sh_ibuf(2) = np2
          sh_ibuf(3) = layout_type
          sh_ibuf(4) = niter
      end if
      sync all
      if (me .ne. 0) then
...
c---------------------------------------------------------------------
c Set images in my row (me1) and column (me2)
c---------------------------------------------------------------------
      allocate( row_images(np2), col_images(np1))
      do i=1,np2
        row_images(i) = (me1+1) + (i-1)*np1
      end do
      do i=1,np1
        col_images(i) = (me2+1) + (i-1)*np2
      end do
...
c---------------------------------------------------------------------
     ...
     >                '        totcomm ' /
      ...
      call coreduce_r8(CAF_SUM, t1(1:t_max+2), tsum(1:t_max+2))
      call coreduce_r8(CAF_MIN, t1(1:t_max+2), tming(1:t_max+2))
      call coreduce_r8(CAF_MAX, t1(1:t_max+2), tmaxg(1:t_max+2))
...
c---------------------------------------------------------------------
      ...
      t(1) = chk
      call coreduce_c8(CAF_SUM, t, result_image=1)
      allchk = t(1)
